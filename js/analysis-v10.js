/* ===========================
   CURETEMBIO AI 피부 분석 엔진 v10.1 HOTFIX
   2026-01-29 - 교육 블록 임시 비활성화 (v20.0 재등장 예정)
   =========================== */

console.log('🚀 [v10.1 HOTFIX] CURETEMBIO AI 분석 엔진 로드 완료 - 교육 블록 비활성화');
...
async function performSimpleAnalysis(imageData) {
    ...
    const getScore = (key, minRandom, maxRandom) => {
        ...
        // 🆕 [v25.1] 점수 범위 확대: 50-85점 → 40-95점
        const brightness = detectedFace.brightness || 128;
        let score = 40 + (brightness / 255) * 55;  // 40-95점 범위
        score += (Math.random() - 0.5) * 30;       // ±15점 변동
        return Math.max(minRandom, Math.min(score, maxRandom));
    };
    
    // 🆕 [v3.5.1] 안전한 번역 - fallback 없이 반드시 번역 키 사용
    const metrics = [
        { name: t('metric_6'), icon: '💧', description: t('metric_6_desc'), score: getScore('moisture', 65, 85), key: 'metric_6' },
        { name: t('metric_5'), icon: '💪', description: t('metric_5_desc'), score: getScore('elasticity', 60, 80), key: 'metric_5' },
        { name: t('metric_4'), icon: '📏', description: t('metric_4_desc'), score: getScore('wrinkles', 65, 85), key: 'metric_4' },
        { name: t('metric_10'), icon: '🔍', description: t('metric_10_desc'), score: getScore('pores', 70, 90), key: 'metric_10' },
        { name: t('metric_3'), icon: '🎨', description: t('metric_3_desc'), score: getScore('pigmentation', 60, 80), key: 'metric_3' },
        { name: t('metric_8'), icon: '🔴', description: t('metric_8_desc'), score: getScore('redness', 65, 85), key: 'metric_8' },
        { name: t('metric_9'), icon: '🔴', description: t('metric_9_desc'), score: getScore('trouble', 70, 90), key: 'metric_9' },
        { name: t('metric_texture'), icon: '✨', description: t('metric_texture_desc'), score: getScore('texture', 65, 85), key: 'metric_texture' },
        { name: t('metric_1'), icon: '🎯', description: t('metric_1_desc'), score: getScore('toneUniformity', 60, 80), key: 'metric_1' },
        { name: t('metric_7'), icon: '🛡️', description: t('metric_7_desc'), score: getScore('sensitivity', 65, 85), key: 'metric_7' }
    ];
    ...
    if (typeof getProductRecommendations_v21 === 'function') {
        ...
        console.warn('⚠️ [v21.0] 신규 추천 함수 없음, v20.0 폴백 사용');
        productRecommendations = getSimpleProductRecommendations(metrics);  // v20.0 폴백
    }
...
/* ===========================
   🎯 v20.0 BETA - 제품 추천 알고리즘 개선
   - 40점대: 프리미엄 + 프리미엄 (강력 집중 케어)
   - 50점대: 프리미엄 + RED (집중 케어)
   - 60점대: 프리미엄 + RED (복합 케어)
   =========================== */
function getSimpleProductRecommendations(metrics) {
    console.log('🎯 [v10.0] 스마트 제품 추천 시작');
    ...
    console.log('📊 낮은 점수 지표:', {
        lowest: `${lowestMetric.key}: ${lowestMetric.score}점`,
        second: `${secondLowestMetric.key}: ${secondLowestMetric.score}점`,
        third: `${thirdLowestMetric.key}: ${thirdLowestMetric.score}점`
    });
...
function determineProductNumber(metricKey) {
    const mapping = {
        ...
        'metric_8': 7,   // 홍조
        'metric_9': 7,   // 트러블
        'metric_10': 5,  // 🆕 [v25.1] 모공 → 5번으로 이동 (탄력 라인)
        'metric_texture': 3  // 🆕 [v25.1] 텍스처 → 3번으로 이동 (미백 라인)
    };
...
function displaySimpleResult(image, analysis) {
    ...
    if (dateElem) {
        // 🔥 CRITICAL FIX: 현재 언어에 맞는 날짜 형식 사용
        ...
        dateElem.textContent = new Date(analysis.date).toLocaleDateString(locale);
    }
            ...
            <div class="card" style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.15), rgba(22, 163, 74, 0.15)); border: 2px solid #22C55E; margin-bottom: 24px;">
                ...
                <div style="background: rgba(220, 20, 60, 0.1); padding: 12px; border-radius: 8px; border-left: 3px solid var(--red-primary);">
                    <p style="color: var(--white-secondary); margin: 0; font-size: 0.875rem; line-height: 1.6;">
                        <i class="fas fa-lightbulb" style="color: #FFD700; margin-right: 6px;"></i> <strong style="color: var(--white-primary);"><span data-i18n="score_note_title">${t('score_note_title')}</span></strong> 
                        <span data-i18n-html="score_note_desc">${t('score_note_desc')}</span>
                    </p>
                </div>
            </div>
    ...
    // 🔥 CRITICAL FIX: 동적 생성된 HTML에 대해 i18n 업데이트 실행
    if (typeof window.updatePageText === 'function') {
        console.log('🌐 [HOTFIX] i18n 업데이트 실행...');
        setTimeout(() => {
            window.updatePageText();
            console.log('✅ [HOTFIX] i18n 업데이트 완료');
        }, 100);
    }
}
            ...
            <div class="card-body" style="color: var(--white-secondary); line-height: 1.8;">
                ...
                ${content.keyPoints && content.keyPoints.length > 0 ? `
                    <div style="background: rgba(0,0,0,0.3); padding: 16px; border-radius: 8px; margin-bottom: 16px;">
                        <h4 style="color: #A5B4FC; margin-bottom: 12px; font-size: 1rem;"><i class="fas fa-lightbulb" style="color: #FFD700; margin-right: 6px;"></i> 핵심 포인트</h4>
        ...
        <div class="card" style="background: linear-gradient(135deg, rgba(220, 20, 60, 0.15), rgba(139, 0, 0, 0.15)); border: 2px solid var(--red-primary); margin-top: 24px;">
            ...
            <div style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.15), rgba(16, 185, 129, 0.15)); padding: 20px; border-radius: 12px; margin-bottom: 24px; border: 2px solid #22C55E;">
                <h4 style="color: #22C55E; margin-bottom: 12px; font-size: 1.1rem; font-weight: 700; text-align: center;">
                    ...
                    <span data-i18n="usage_tip_title"><i class="fas fa-lightbulb" style="color: #FFD700; margin-right: 6px;"></i> 사용 팁</span>
                </h4>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                ...
                <div style="background: rgba(0,0,0,0.3); border-radius: 12px; padding: 16px; text-align: center;">
                    <div style="position: relative; cursor: pointer;" onclick="window.showImageModal('${rec.premium.detailImage || rec.premium.image}', '${translateProductName(rec.premium.name)}')">
                        <img src="${rec.premium.detailImage || rec.premium.image}" alt="${translateProductName(rec.premium.name)}" style="width: 100%; max-width: 240px; height: 280px; object-fit: contain; border-radius: 8px; margin-bottom: 12px;">
                ...
                <div style="background: rgba(0,0,0,0.3); border-radius: 12px; padding: 16px; text-align: center;">
                    <div style="position: relative; cursor: pointer;" onclick="window.showImageModal('${rec.red.detailImage || rec.red.image}', '${translateProductName(rec.red.name)}')">
                        <img src="${rec.red.detailImage || rec.red.image}" alt="${translateProductName(rec.red.name)}" style="width: 100%; max-width: 240px; height: 280px; object-fit: contain; border-radius: 8px; margin-bottom: 12px;">
                        <div style="position: absolute; top: 8px; right: 8px; background: rgba(0,0,0,0.7); color: white; padding: 8px 12px; border-radius: 20px; font-size: 0.75rem;">
                            <i class="fas fa-search-plus" style="margin-right: 4px;"></i><span data-i18n="image_zoom">${t('image_zoom')}</span>
                        </div>
                    </div>
